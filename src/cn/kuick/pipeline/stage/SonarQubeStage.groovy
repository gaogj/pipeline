	...
	def run() {
		...
		this.script.node("aliyun327-test") {
	        ...
	        if (projectType == "java") {
                ...
                this.script.sh "/opt/sonar-scanner-3.0.3.778-linux/bin/sonar-scanner -Dsonar.host.url=https://sonar.kuick.cn   -Dsonar.login=${sonarToken} -Dsonar.projectKey=${serverName}  -Dsonar.sourceEncoding=UTF-8 -Dsonar.exclusions=libs/* -Dsonar.sources=."

...
class BuildBaseImageStage implements Serializable {
	...
	//	def reportPath = "./clair-report.json"
		...
		try {
			...
			this.script.emailext attachmentsPattern: reportPath,body: "附件为镜像漏洞扫描结果 At ${imageName}", recipientProviders: [[$class: 'DevelopersRecipientProvider'], [$class: 'RequesterRecipientProvider']], subject: "${name}-server 镜像扫描结果 At buildId(#${buildId})",to: toMail
			// cc: 'devops@kuick.cn'

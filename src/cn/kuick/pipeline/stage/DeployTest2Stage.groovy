...
class DeployTest2Stage implements Serializable {
    ...
    def run() {
        ...
        if (deployNode == "aliyun345") {
            deployNode = "aliyun345-build"
        } else {
            deployNode = "${deployNode}-test2"
        }
        ...
        docker.withRegistry('https://registry.kuick.cn', 'kuick_docker_registry_login') {
            this.script.node("${deployNode}") {
                this.script.echo "login to ${deployNode}"
                ...
                if (serverName == "kafka" || serverName == "kafka2" || serverName == "zookeeper") {
                    ...
                    this.script.sh "release/docker/${deployNode}/deploy.sh ${version}"
